...
const SITE_EXPERIMENT_IDS = new Set(["sundays", "design-meetup"]);

function isSiteExperiment(projectId: string): boolean {
  return SITE_EXPERIMENT_IDS.has(projectId);
}

type SiteExperimentConfig = {
  href: string;
  label: string;
  headerAction: "sundays-mark" | "visit-site";
};

function siteExperimentConfig(projectId: string): SiteExperimentConfig | undefined {
  switch (projectId) {
    case "sundays":
      return {
        href: "https://sundays.rsvp",
        label: "sundays.rsvp",
        headerAction: "sundays-mark",
      };
    case "design-meetup":
      return {
        href: "https://design-meetup-web.vercel.app/",
        label: "Visit Site",
        headerAction: "visit-site",
      };
    default:
      return undefined;
  }
...
function ExperimentCta({ href }: { href?: string }) {
  ...
  if (/^https?:\/\//.test(trimmed)) {
    return <VisitSiteButton href={trimmed} size="md" className="w-fit mt-1" />;
  }
  return <TryItOutButton href={trimmed} className="w-fit mt-1" />;
}

function SiteLabelLink({ href, label }: { href: string; label: string }) {
  return (
    <a
      href={href}
      target="_blank"
...
function ViewOnXButton({ href, className }: { href: string; className?: string }) {
  return (
    <a
      href={href}
      target="_blank"
...
function SundaysMarkLink({ href, label }: { href: string; label: string }) {
  return (
...
function SiteExperimentHeaderActions({
  project,
  site,
  variant,
}: {
  project: ExperimentProject;
  site: SiteExperimentConfig;
  variant: "desktop" | "mobile";
}) {
  const siteAction =
    site.headerAction === "sundays-mark" && variant === "desktop" ? (
      <SundaysMarkLink href={site.href} label={site.label} />
    ) : site.headerAction === "visit-site" && variant === "desktop" ? (
      <VisitSiteButton href={site.href} />
    ) : (
      <SiteLabelLink href={site.href} label={site.label} />
    );

  const xAction = project.xLink ? (
    <ViewOnXButton
      href={project.xLink}
      className={variant === "mobile" ? "relative self-start whitespace-nowrap" : "whitespace-nowrap"}
    />
  ) : null;
  ...
  return (
    <div className="font-['Michelle',sans-serif] min-h-full w-full box-border flex flex-col gap-6 px-6 py-16 md:px-16 md:py-20 text-[#18181b]">
      <header className="flex flex-col gap-2 max-w-2xl">
        ...
        <ExperimentCta href={project.tryItOutHref} />
      </header>
      {hasMedia ? (
...
function SiteExperimentEmbed({ project, isFullscreen = false, isScrolled = false, isPastHero = false, onCollapse }: { project: ExperimentProject; isFullscreen?: boolean; isScrolled?: boolean; isPastHero?: boolean; onCollapse?: () => void }) {
  const [videoReady, setVideoReady] = useState(false);
  const navigate = useNavigate();
  const site = siteExperimentConfig(project.id);
    ...
    <div className={clsx(
      ...
      {isFullscreen && (
        ...
        >
          ...
          <Breadcrumb
            projectName={typeof project.title === "string" ? project.title : "Work"}
        ...
        <header className="flex flex-col">
          ...
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="hidden md:flex flex-wrap gap-2 shrink-0 items-center">
              <SiteExperimentHeaderActions project={project} site={site} variant="desktop" />
            </div>
          </div>
          {/* Mobile: buttons below description — blue first, then ghost */}
          <div className="flex md:hidden flex-wrap gap-0.5 mt-2">
            <SiteExperimentHeaderActions project={project} site={site} variant="mobile" />
          </div>
        </header>
        {project.toolCategories && project.toolCategories.length > 0 ? (
...
function SiteExperimentMobileEmbed({ project }: { project: ExperimentProject }) {
  const [videoReady, setVideoReady] = useState(false);
  const site = siteExperimentConfig(project.id);
  ...
  return (
    <div className="sundays-mobile-embed content-stretch flex flex-col gap-4 px-6 py-5 relative shrink-0 w-full">
      ...
      {/* Buttons: Visit Site / sundays.rsvp, then View on X, below media */}
      <div className="flex flex-wrap gap-2 items-center justify-end py-1">
        <SiteLabelLink href={site.href} label={site.label} />
        {project.xLink && (
          <ViewOnXButton href={project.xLink} className="relative self-start whitespace-nowrap" />
        )}
      </div>
    </div>
  );
...
export default function ExperimentModal({ projectId, project, onClose, onExpandToFullscreen, onCollapseFromFullscreen, onBookSlugChange, bookSlug, initialFullscreen = false }: ExperimentModalProps) {
  ...
  // Site experiments (Sundays, Design Meetup) stay in the popup on mobile.
  useEffect(() => {
    if (isSiteExperiment(projectId) && initialFullscreen) {
      const checkMobile = () => {
        const isMobile = window.innerWidth < 768;
        if (isMobile) {
    ...
    switch (projectId) {
      ...
      case 'design-meetup':
        if (isMobile && !isFullscreen) {
          return <SiteExperimentMobileEmbed project={project} />;
        }
        return <SiteExperimentEmbed project={project} isFullscreen={isFullscreen} isScrolled={isScrolled} isPastHero={isPastHero} onCollapse={handleCollapse} />;
      default:
        return <GenericExperimentEmbed project={project} />;
    }
  };
  ...
  return (
    <div className={clsx(
        ...
        className={clsx(
          ...
          isFullscreen 
            ...
            : clsx(
              ...
              isMobile && isSiteExperiment(projectId) ? "max-h-[90vh]" : "h-[90vh]"
            ),
          isVisible 
      ...
      >
        ...
        {/* Hidden on mobile for site experiments (no fullscreen on mobile) */}
        {!isFullscreen && (
          <div className={clsx(
            "absolute top-0 left-0 z-[60] pointer-events-none pl-6 pt-6",
            isSiteExperiment(projectId) && 'max-md:hidden'
          )}>
        ...
        {/* Info button fixed top right - only in popup mode, hidden for site experiments (content already visible) */}
        {!isFullscreen && !isSiteExperiment(projectId) && (
          <div className={clsx(
          ...
          <Suspense fallback={<ExperimentLoading />}>
            <div 
              className={clsx(
                ...
                !isFullscreen && isSiteExperiment(projectId) && "max-md:overflow-hidden"
              )}
              style={{ 
            ...
            >
              ...
              <div 
                ...
                style={{ 
                  ...
                  transform: !isFullscreen && contentScale < 1 && !(isMobile && isSiteExperiment(projectId)) ? `scale(${contentScale})` : undefined,
                  width: !isFullscreen && contentScale < 1 && !(isMobile && isSiteExperiment(projectId)) ? `${100 / contentScale}%` : undefined,
                }}
              >
                {renderExperiment()}
              </div>
