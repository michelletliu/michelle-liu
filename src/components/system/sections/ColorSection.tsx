import { colorGroups, uniformTag } from "../tokens";
        ...
        {colorGroups.map((group) => {
          ...
          return (
            <div key={group.id}>
              ...
              <div className="mb-6 flex h-9 overflow-hidden rounded-lg ring-1 ring-inset ring-zinc-900/5">
                {group.colors.map((c) => (
                  <div
              ...
              >
                {group.colors.map((c) => (
                  <div key={c.name + c.value} className="flex items-start gap-3">
                    <div
                      className="mt-0.5 h-9 w-9 shrink-0 rounded-lg ring-1 ring-inset ring-zinc-900/5"
                      style={{ backgroundColor: c.value }}
                    />
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between gap-2">
                        <span className="truncate text-base font-medium text-zinc-700">{c.name}</span>
                        {!groupTag && <TagChip tag={c.tag} />}
                      </div>
