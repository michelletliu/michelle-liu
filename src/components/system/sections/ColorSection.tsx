...
export default function ColorSection() {
  return (
    <Section
      id="color"
      title="Color"
      subtitle="A tinted-neutral gray scale carries the whole site; blue-500 is the single interactive accent. Everything else is status or one-off."
    >
              ...
              <div key={c.name + c.value} className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-2">
                    <span className="truncate text-base font-medium text-gray-700">{c.name}</span>
                    <TagChip tag={c.tag} />
                  </div>
                  <div className="mt-0.5 flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
                    <code className="font-mono text-sm uppercase text-gray-400">{c.value}</code>
                    {c.className && (
                      <code className="font-mono text-sm text-gray-300">{c.className}</code>
                    )}
                  </div>
                  <p className="mt-1 text-sm leading-snug text-gray-400 text-pretty">{c.usage}</p>
                </div>
              </div>
            ))}
          </div>
