  ...
  return (
    <section id={id} className="scroll-mt-24 border-t border-zinc-100 pt-16 pb-4">
      <div className="mb-10 flex flex-col gap-2">
        <h2 className="font-['Michelle',sans-serif] text-2xl font-medium tracking-tight text-zinc-900">
          {title}
        </h2>
        {subtitle && (
          <p className="max-w-2xl font-['Michelle',sans-serif] text-base font-normal leading-relaxed text-zinc-500 text-pretty">
            {subtitle}
          </p>
        )}
      </div>
...
export function SubLabel({ children, note }: { children: ReactNode; note?: string }) {
  ...
  return (
    <div id={id} className="mb-5 mt-20 scroll-mt-28 first:mt-0">
      <h3 className="text-base font-medium text-zinc-500">{children}</h3>
      {note && (
