 ...
 * for most cards that lands somewhere representative. Clips that assemble UI
 * from an empty first frame do not: the midpoint is the settled layout, so
 * ProjectMedia crossfades that poster out when playback starts at t=0 and the
 * card looks like it fades out, then slides in. Projects listed here pin the
